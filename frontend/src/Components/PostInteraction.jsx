...
const LikeInteraction = (props) => {
    ...
    const likePost = (event) => {
        ...
        fetch(apiBaseUrl + "/api/posts/like/" + props.post._id, {
            ...
            .then(resp => resp.json())
            .then(data => {
                props.interactionChange ? props.setInteractionChange(false) : props.setInteractionChange(true)
                console.log("likeToggle used...", data)
            })
    }

    return (
        <div onClick={likePost} className={props.post.likedByUser ? "heart-liked" : "heart-dislike"}>
